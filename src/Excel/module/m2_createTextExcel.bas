...
Private Sub Initialize()
    ...
    ' outputセル
    Dim srcWs As Worksheet: Set srcWs = ActiveSheet ' 現在のアクティブなシートを取得
    Dim dstWs As Worksheet: Set dstWs = Worksheets.Add(After:=srcWs) ' 現在のシートの右側に新しいシートを追加
    Dim ws As Worksheet
    Dim find As Boolean
    For Each ws In Worksheets
        If ws.name = srcWs.name & "_単体テスト" Then
            find = True
            Exit For
        End If
    Next
    If find = False Then
        dstWs.name = srcWs.name & "_単体テスト"
    End If
